...
3. **テーブル一覧（スキーマビューモード）**: 接続成功後、テーブル一覧が表示されます
   - `j`/`k` または `↑`/`↓` でテーブルを選択
   - 右ペインに選択したテーブルの詳細（カラム、インデックス、親子関係）が表示されます
   - `Enter` でデータ表示モードに切り替え
4. **データ表示モード**: テーブルのデータが表示されます
   - データはPRIMARY KEYでソートされています
   - `j`/`k` または `↑`/`↓` でデータ行をスクロール（最大1000行）
   - カラム幅はデータに応じて自動調整されます（最大24文字）
   - `Esc` でスキーマビューモードに戻る
5. **終了**: `q` で終了
