# UI仕様

## ステップ1: エディション選択

```
╭── Dito ─────────────────────────────────────────────────────────╮
│                                                                 │
│  Select Connection                                              │
│   > Oracle NoSQL Cloud Service                                  │
│     On-Premise                                                  │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | Select: <enter>                               │
╰─────────────────────────────────────────────────────────────────╯
```

## ステップ2-a: Cloud接続設定

```
╭── Dito ─────────────────────────────────────────────────────────╮
│                                                                 │
│  Cloud Connection                                               │
│  Region:        [ us-ashburn-1                  ]               │
│  Compartment:   [ ocid1.compartment.oc1..aaaaaa ]               │
│                                                                 │
│  Auth Method:                                                   │
│   > (*) OCI Config Profile (default)                            │
│     ( ) Instance Principal                                      │
│     ( ) Resource Principal                                      │
│                                                                 │
│  Config File:   [ DEFAULT                       ]               │
│                                                                 │
│   > Test Connection                                             │
│     Connect                                                     │
│                                                                 │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | Toggle: <space> | Edit/Execute: <enter> | ... │
╰─────────────────────────────────────────────────────────────────╯
```

**変更点:**
- テキストフィールドは直接編集できません
- テキストフィールドにフォーカスして`Enter`を押すとテキスト入力ダイアログが開きます

### テキスト入力ダイアログ

```
╭────────────────────────────────────────╮
│  Region                                │
│                                        │
│  us-ashburn-1_                         │
│                                        │
│  Save: <enter> | Cancel: <esc>         │
╰────────────────────────────────────────╯
```

## ステップ2-b: On-Premise接続設定

```
╭── Dito ─────────────────────────────────────────────────────────╮
│                                                                 │
│  On-Premise Connection                                          │
│  Endpoint: [ localhost              ]                           │
│  Port:     [ 8080   ]                                           │
│  Secure:   [ ] HTTPS/TLS                                        │
│                                                                 │
│   > Test Connection                                             │
│     Connect                                                     │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│ Connected                                                       │
├─────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | Toggle: <space> | Edit/Execute: <enter> | ... │
╰─────────────────────────────────────────────────────────────────╯
```

**変更点:**
- テキストフィールドは直接編集できません
- テキストフィールドにフォーカスして`Enter`を押すとテキスト入力ダイアログが開きます

## ステップ3: テーブル一覧

接続成功後、左ペインにテーブル一覧、右ペインに選択したテーブルの詳細を表示します。

```
╭── Dito ─────────────────────────────────────────────────────────────────────────────╮
│                                                        Connected to localhost:8080  │
├─────────────────────────────────┬───────────────────────────────────────────────────┤
│  Tables (7)                     │  Table:    users                                  │
│  > users                        │  Parent:   -                                      │
│    users.addresses              │  Children: addresses, phones                      │
│    users.phones                 │                                                   │
│    products                     │  Columns:                                         │
│    products.reviews             │    id           INTEGER (Primary Key)             │
│    orders                       │    name         STRING                            │
│    orders.items                 │    email        STRING                            │
│                                 │    age          INTEGER                           │
│                                 │    created_at   TIMESTAMP(3)                      │
│                                 │                                                   │
│                                 │  Indexes:                                         │
│                                 │    email_idx    (email)                           │
│                                 │                                                   │
│                                 │                                                   │
│                                 │                                                   │
├─────────────────────────────────┴───────────────────────────────────────────────────┤
│ Connected: localhost:8080                                                           │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | View Data: <enter> | Back: <esc>                                  │
╰─────────────────────────────────────────────────────────────────────────────────────╯
```

**特徴:**
- 左ペイン: テーブル一覧（固定幅30文字）
- 右ペイン: 選択したテーブルの詳細（残りの幅）
- 親子テーブルの階層を表現（インデントなし、ドット記法）
- システムテーブル（`SYS$*`）は非表示
- テーブル数を表示
- 選択中のテーブルは `>` でハイライト

**親子テーブルの表示例:**

子テーブル選択時：
```
╭── Dito ─────────────────────────────────────────────────────────────────────────────╮
│                                                        Connected to localhost:8080  │
├─────────────────────────────────┬───────────────────────────────────────────────────┤
│  Tables (7)                     │  Table:    users.addresses                        │
│    users                        │  Parent:   users                                  │
│  > users.addresses              │  Children: -                                      │
│    users.phones                 │                                                   │
│    products                     │  Columns:                                         │
│    products.reviews             │    id           INTEGER (From parent)             │
│    orders                       │    address_id   INTEGER (Primary Key)             │
│    orders.items                 │    type         STRING                            │
│                                 │    street       STRING                            │
│                                 │    city         STRING                            │
│                                 │    state        STRING                            │
│                                 │    postal_code  STRING                            │
│                                 │    country      STRING                            │
│                                 │    is_primary   BOOLEAN                           │
│                                 │                                                   │
│                                 │  Indexes:                                         │
│                                 │    (none)                                         │
├─────────────────────────────────┴───────────────────────────────────────────────────┤
│ Connected: localhost:8080                                                           │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | View Data: <enter> | Back: <esc>                                  │
╰─────────────────────────────────────────────────────────────────────────────────────╯
```

## ステップ4: データ表示

テーブル一覧画面で`Enter`キーを押すと、右ペインが「グリッドビュー」に切り替わります。さらに`Enter`キーを押すと「レコードビュー」に切り替わります。`Esc`キーで前のビューに戻ります。

### グリッドビュー

```
╭── Dito ─────────────────────────────────────────────────────────────────────────────╮
│                                                        Connected to localhost:8080  │
├─────────────────────────────────┬───────────────────────────────────────────────────┤
│  Tables (7)                     │  SELECT * FROM users ORDER BY id                  │
│                                 │  ───────────────────────────────────────────────  │
│  > users                        │    id  name          email              age  cre… │
│    users.addresses              │    ──  ────          ─────              ───  ───… │
│    users.phones                 │  > 1   Alice Smith   alice@example.com  28   202… │
│    products                     │    2   Bob Johnson   bob@example.com    35   202… │
│    products.reviews             │    3   Charlie Bro…  charlie@exampl…    42   202… │
│    orders                       │    4   Diana Princ…  diana@example.c…   31   202… │
│    orders.items                 │    5   Eve Wilson    eve@example.com    29   202… │
│                                 │    6   Frank Mille…  frank@example.c…   45   202… │
│                                 │    7   Grace Lee     grace@example.com  33   202… │
│                                 │    8   Henry Davis   henry@example.com  38   202… │
│                                 │    9   Iris Chen     iris@example.com   27   202… │
│                                 │    10  Jack Thomps…  jack@example.com   41   202… │
├─────────────────────────────────┴───────────────────────────────────────────────────┤
│ Table: users (150 rows)                                                             │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | Scroll H: ←/→ | Detail: <enter> | Edit SQL: e | Back: <esc>       │
╰─────────────────────────────────────────────────────────────────────────────────────╯
```

**特徴:**
- 2ペインレイアウトを維持（左：テーブル一覧、右：データ表示）
- 上部にSQL文を表示（実行したクエリが確認できる）
- `Enter`キーで右ペインをグリッドビューに切り替え
- `Esc`キーで右ペインをスキーマビューに戻す
- **グリッドビュー時:**
  - 左ペインはグレーアウトされ、インアクティブ状態
  - `↑`/`↓`（または `k`/`j`）でデータ行をスクロール
  - `←`/`→`（または `h`/`l`）でカラムを左右にスクロール
  - 選択中の行はシアン色 + 太字 + `>` マーカー
  - テーブルを切り替えるには`Esc`でスキーマビューに戻る必要がある
  - `Enter`でレコードビューに切り替え
  - `e`キーでSQLエディタダイアログを開く（カスタムSQLを編集・実行可能）
- カラム幅はデータに応じて自動調整（最大32文字）、長いデータは省略（`…`）
- 画面高さに応じた動的なビューポート表示
- PRIMARY KEYでソートされたデータを表示（最大1000行取得）
- ステータスエリアにテーブル名と行数を表示
- データ取得エラー時はステータスバーに赤字でエラーメッセージを表示

### スキーマビュー

```
╭── Dito ─────────────────────────────────────────────────────────────────────────────╮
│                                                        Connected to localhost:8080  │
├─────────────────────────────────┬───────────────────────────────────────────────────┤
│  Tables (7)                     │  Table:    users                                  │
│  > users                        │  Parent:   -                                      │
│    users.addresses              │  Children: addresses, phones                      │
│    users.phones                 │                                                   │
│    products                     │  Columns:                                         │
│    products.reviews             │    id           INTEGER (Primary Key)             │
│    orders                       │    name         STRING                            │
│    orders.items                 │    email        STRING                            │
│                                 │    age          INTEGER                           │
│                                 │    created_at   TIMESTAMP(3)                      │
│                                 │                                                   │
│                                 │  Indexes:                                         │
│                                 │    email_idx    (email)                           │
│                                 │                                                   │
├─────────────────────────────────┴───────────────────────────────────────────────────┤
│ Table: users                                                                        │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | View Data: <enter> | Back: <esc>                                  │
╰─────────────────────────────────────────────────────────────────────────────────────╯
```

**操作フロー:**
1. **スキーマビュー**: テーブル選択（`↑`/`↓` または `k`/`j` で移動）
2. `Enter` → グリッドビューに切り替わる、左ペインがグレーアウト
3. **グリッドビュー**: データ行をスクロール（`↑`/`↓` または `k`/`j` で移動、`←`/`→` または `h`/`l` で横スクロール）
4. `Enter` → レコードビューに切り替わる
5. **レコードビュー**: 選択した行のデータを縦に詳細表示（`↑`/`↓` または `k`/`j` で行を切り替え）
6. `Esc` → グリッドビューに戻る
7. もう一度 `Esc` → スキーマビューに戻る（左ペインが通常色に戻る）
8. もう一度 `Esc` → 接続設定画面に戻る

### 空のテーブルのグリッドビュー

```
╭── Dito ─────────────────────────────────────────────────────────────────────────────╮
│                                                        Connected to localhost:8080  │
├─────────────────────────────────┬───────────────────────────────────────────────────┤
│  Tables (7)                     │  SELECT * FROM empty_table ORDER BY id            │
│                                 │  ───────────────────────────────────────────────  │
│  > empty_table                  │    id  name  email                                │
│    users                        │    ──  ────  ─────                                │
│    users.addresses              │                                                   │
│    users.phones                 │            No data found                          │
│    products                     │                                                   │
│    products.reviews             │                                                   │
│    orders                       │                                                   │
│    orders.items                 │                                                   │
│                                 │                                                   │
├─────────────────────────────────┴───────────────────────────────────────────────────┤
│ Table: empty_table (0 rows)                                                         │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | Scroll H: ←/→ | Detail: <enter> | Edit SQL: e | Back: <esc>       │
╰─────────────────────────────────────────────────────────────────────────────────────╯
```

### レコードビュー

```
╭── Dito ─────────────────────────────────────────────────────────────────────────────╮
│                                                        Connected to localhost:8080  │
├─────────────────────────────────┬───────────────────────────────────────────────────┤
│  Tables (7)                     │  SELECT * FROM users ORDER BY id                  │
│                                 │  ───────────────────────────────────────────────  │
│  > users                        │  id           1                                   │
│    users.addresses              │  name         Alice Smith                         │
│    users.phones                 │  email        alice@example.com                   │
│    products                     │  age          28                                  │
│    products.reviews             │  created_at   2024-01-15T10:30:00.000Z            │
│    orders                       │  phone        +1-555-0101                         │
│    orders.items                 │  address      123 Main St                         │
│                                 │  city         New York                            │
│                                 │  state        NY                                  │
│                                 │  postal_code  10001                               │
│                                 │  country      USA                                 │
│                                 │  company      Tech Corp                           │
│                                 │  title        Software Engineer                   │
├─────────────────────────────────┴───────────────────────────────────────────────────┤
│ Table: users (Row 1 of 150)                                                         │
├─────────────────────────────────────────────────────────────────────────────────────┤
│  Navigate: ↑/↓ | Back: <esc>                                                        │
╰─────────────────────────────────────────────────────────────────────────────────────╯
```

**特徴:**
- 選択した行のデータを縦に詳細表示
- カラム名を太字+シアン色で表示
- 全てのカラムが表示される（横スクロールの制限なし）
- `↑`/`↓`（または `k`/`j`）で次/前の行に切り替え
- `Esc`でグリッドビューに戻る
- ステータスエリアに行番号（何番目の行か）を表示

### SQLエディタダイアログ

グリッドビューで`e`キーを押すとSQLエディタダイアログが開きます。

```
╭───────────────────────────────────────────────────────────╮
│  SQL Editor                                               │
│                                                           │
│  SELECT * FROM users WHERE age > 30_                      │
│                                                           │
│                                                           │
│                                                           │
│  Execute: ctrl+r | Cancel: <esc>                          │
╰───────────────────────────────────────────────────────────╯
```

**特徴:**
- カスタムSQLクエリを編集・実行可能
- `Ctrl+R`で実行、結果がグリッドビューに表示される
- `Enter`キーは改行として機能（SQL内で改行可能）
- `Esc`キーでキャンセル
- 実行したカスタムSQLはグリッド上部に表示される

**将来実装予定:**
- 左ペイン折りたたみ機能（例: `Ctrl+B`）でデータ表示領域を拡大
